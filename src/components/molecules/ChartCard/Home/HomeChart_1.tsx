...
import { ChartMenu, LineChart } from "@/src/components/atoms";
import httpServices from "@/src/utils/httpServices";
import { App_Config } from "@/config";
import { setHomeChart_1 } from "@/src/store/slices/dashboard";
...
const HomeChart_1 = () => {
  ...
  const chartData = useSelector(
    (state: RootState) => state.dashboard.home.homeChart_1
  );
  //-------------------------------------------------------------------------//
  const fetchData = async () => {
    try {
      const res = await httpServices.post(
        `${App_Config.API_BASE_URL}/api/dashboard/home/get-home-chart-1`,
        {
  ...
  useEffect(() => {
    fetchData().then((apiData) => {
      if (apiData !== undefined) dispatch(setHomeChart_1(apiData));
    });
  }, []);
  ...
  return (
    <div
      className="col-span-2 rounded-2xl h-fit lg:h-[240px] mb-5 text-[#2B2B2B] font-semibold w-full"
